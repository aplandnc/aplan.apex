...
export default function RegisterPage() {
  ...
  // - rejected: 반려 -> 반려사유 표시 + 기존값 유지 + 수정 가능(팝업X)
  ...
  useEffect(() => {
    ...
    // rejected/pending/approved/checking/needs_login 중에는 팝업 닫기
    if (privacyOpen) setPrivacyOpen(false);

    // pending/rejected에서는 기존 UX 유지 (이미 동의된 것으로 간주)
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        // ✅ status 우선, legacy approved fallback
        ...
        // pending(기본) → 가입화면 그대로 + 전체 비활성화 + 문구
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ rejected(또는 기존 row 존재)면 insert 대신 update로 재신청
      if (staffRowId) {
      ...
      // ✅ 서버에 실제로 pending 반영됐는지 즉시 확인 (안 바뀌면 승인목록 안 뜸)
...
if (nowStatus !== 'pending') {
  ...
  // 여기서 pending 오버레이로 넘어가버리면 사용자가 "된 줄" 착각하니 return
  return;
}
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4 apex-cursorfix">
      ...
      <div className="relative max-w-md mx-auto">
        {/* ✅ 반려 사유 표시 (rejected일 때만) */}
        {isRejected && rejectedReason?.trim() && (
          <div className="mb-4 rounded-2xl border border-red-200 bg-red-50 p-4 shadow-sm">
            <div className="text-sm font-semibold text-red-800 select-none">반려 사유</div>
            <div className="text-sm text-red-700 mt-1 whitespace-pre-wrap">{rejectedReason}</div>
          </div>
        )}
